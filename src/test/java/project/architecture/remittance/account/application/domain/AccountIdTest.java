...
@DisplayName("계좌 아이디 도메인 테스트")
class AccountIdTest {

    @ParameterizedTest
    @ValueSource(longs = {0, -1, -2})
    @DisplayName("0이하의 잘못된 값이 들어오면 IllegalArgumentException을 발생시킨다.")
    void 계좌_아이디_생성_실패_테스트(Long parameter) {
        assertThatThrownBy(() -> new AccountId(parameter))
    ...
    void 계좌_아이디_생성_실패_테스트_유스_케이스(Long parameter) {
        assertThatThrownBy(() -> new AccountId(parameter))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void 계좌_아이디_값_비교_테스트(Long parameter) {
        Assertions.assertEquals(new AccountId(parameter), new AccountId(parameter));
    }
}
